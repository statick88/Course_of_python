...
Imagina que tienes una **lista de compras** en tu teléfono. No es un solo
producto — es una **colección ordenada** de cosas que necesitas. En Python, las
listas hacen exactamente eso: guardan múltiples valores en un solo lugar,
manteniendo el orden en que los pusiste.
...
1. Una variable simple solo puede tener un valor a la vez. Si asignas otro, el
anterior se pierde.
2. Una lista puede contener muchos valores, y todos se mantienen accesibles por
su posición.
...
Crear una lista es tan simple como poner valores entre corchetes `[]`, separados
por comas:
...
1. Los corchetes `[]` crean una lista. Cada valor entre comas es un **elemento**
de la lista.
2. Las listas pueden contener cualquier tipo de dato. Esta lista tiene números
decimales (`float`).
3. `[]` es una lista vacía. Es como tener una caja sin nada adentro — lista para
usar.
...
A diferencia de otros lenguajes, Python te permite mezclar tipos en la misma
lista. No es lo más común, pero es posible:
...
1. `perfil` contiene un `str`, un `int`, un `bool` y un `float` — todos en la
misma lista.
...
2. `list("Python")` crea `['P', 'y', 't', 'h', 'o', 'n']` — cada carácter es un
elemento.
3. `range(5)` genera números del 0 al 4. `list()` los convierte en `[0, 1, 2, 3,
4]`.
4. Una tupla es como una lista pero inmutable (no se puede modificar). La
...
`range()` es tu aliado cuando necesitas secuencias de números. No crea una lista
directamente — crea un **objeto rango** que genera números bajo demanda:
...
3. `range(0, 20, 4)` genera: 0, 4, 8, 12, 16. El `4` es el paso (salta de 4 en
4).
4. `range(10, 0, -2)` genera: 10, 8, 6, 4, 2. El paso negativo cuenta hacia
atrás.
...
1. Sin comas, Python concatena los strings: `"rojo" "verde" "azul"` se convierte
en `"rojoverdeazul"`. No es un error de sintaxis, pero probablemente no es lo
que querías.
2. Con comas, cada color es un elemento separado: `['rojo', 'verde', 'azul']`.
3. Los paréntesis `()` crean una **tupla**, no una lista. Las tuplas son
inmutables.
4. Los corchetes `[]` crean una **lista** mutable que puedes modificar.
5. Python busca variables llamadas `a`, `b`, `c`. Si no existen, obtienes
`NameError`.
...
**Anterior:** [Módulo 4](../modulo-04/index.qmd) | **Siguiente:** [5.2 Indexar y
slicing](02-indexar-slicing.qmd)
